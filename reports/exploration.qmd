...
charities |> 
  count(category_code, sub_category_code) 

t3010_clusters <- clusters |> 
  filter(data_source == "cra") |> 
  select(-slc_line) 

missing <- t3010_clusters |> 
  anti_join(charities, by = c("category_code", "sub_category_code"))  |> 
  mutate(in_data = "Not in T3010 data")

present <- t3010_clusters |> 
  semi_join(charities, by = c("category_code", "sub_category_code")) |> 
  mutate(in_data = "In T3010 data")

bind_rows(missing, present) |> 
  select(-data_source) |> 
  group_by(in_data) |> 
  gt(groupname_col = "in_data", rowname_col = "cluster") |> 
  gt::tab_stub_indent(rows = everything()) |> 
  gt::tab_style(
    style = gt::cell_text(weight = "bold"),
    locations = gt::cells_row_groups(groups = everything())
